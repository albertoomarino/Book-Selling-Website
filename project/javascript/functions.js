...
--
Codice JavaScript che gestisce tutte le funzioni
*/
            ...
            if (data.length === 0) {
                ...
                for (let i in data) {
                    ...
                    $("#card").append(
                        ...
                        // Ci assicuriamo che la proprietà "price" esista prima di utilizzare il metodo "toFixed"
                        "<p id='book_price' title=\"Book's price\">" + (data[i].price ? data[i].price.toFixed(2) : "") + " €</p>" +
                        "</div>" +
...
/**
 * Codice relativo alla funzione "show_select_titles"
 */
$(document).ready(function () {
    $.ajax({
        url: "../php/functions.php",
...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (response) {
            ...
            if (data.length === 0) {
                $("#empty_view").append(
                    "<p id='empty_view'>There are no books for sale at the moment!</p>"
                );
            } else {
                for (i in data) {
                    $("#selected_book").append(
                        ...
                        // Ci assicuriamo che la proprietà "price" esista prima di utilizzare il metodo "toFixed"
                        "<p id='descr_price' title=\"Book's price\">" + "<span id='descr'>Book's price:</span>" + " " + (data[i].price ? data[i].price.toFixed(2) : "") + " €" + "</p>" +
            ...
            if (data.length === 0) {
                ...
                for (i in data) {
                    ...
                    $("#basket_book_box").append(
                        ...
                        // Ci assicuriamo che la proprietà "price" esista prima di utilizzare il metodo "toFixed"
                        "<p class='basket_book_price' id='basket_book_price' title=\"Book's price\">" + (data[i].price ? data[i].price.toFixed(2) : "") + " €" + "</p>" +
 ...
 *
 * La funzione si comporta nel seguente modo:
 ...
 * - Se la quantità è maggiore di 1, decrementa la quantità e aggiorna il valore
 ...
 * - Ottiene il valore del prezzo totale come un numero decimale e rimuove il testo "Total price: " e il simbolo "€"
...
function decrement_price() {
    ...
    if (quantity > 1) {
        ...
        // "closest()" cerca l'elemento che corrisponda al selettore specificato
 ...
 *
 * La funzione si comporta nel seguente modo:
 * - Trova l'elemento della quantità corrispondente al pulsante di incremento e incrementa il valore di 1.
 * - Aggiorna il testo dell'elemento della quantità con il nuovo valore.
 * - Trova l'elemento del prezzo corrispondente alla carta del carrello e ottiene il valore del prezzo.
 * - Trova l'elemento del prezzo totale e ottiene il valore attuale del prezzo totale.
 * - Aggiunge il prezzo dell'elemento alla variabile del prezzo totale.
 * - Aggiorna il testo dell'elemento del prezzo totale con il nuovo valore.
 */
function increment_price() {
    const quantityElement = $(this).siblings(".quantity");
    let quantity = parseInt(quantityElement.data("quantity")); // Valore della quantità come intero
    quantity += 1;
    quantityElement.data("quantity", quantity);
    quantityElement.text(quantity);
    // "closest()" cerca l'elemento che corrisponda al selettore specificato
...
$(document).ready(function () {
    $.ajax({
        ...
        success: function (response) {
            let data = response;
            // Se non ci sono recensioni, nascondo la section e viene mostrato il messaggio
            if (data.length === 0) {
